...
// ==========================================
const DraggableGameCard = ({ game, onClick, onRemove, onRate, t }) => {
...
// ==========================================
const KanbanColumn = ({ id, title, games, onGameClick, onRemove, onRate, t }) => {
...
const Library = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setShowChart(true), 150); // Animasyon sonrası çiz (150ms gecikme)
  ...
  const stats = {
    ...
    playing: libraryGames.filter(g => g.status === 'Playing').length,
    completed: libraryGames.filter(g => g.status === 'Completed').length,
    backlog: libraryGames.filter(g => g.status === 'Backlog').length,
    dropped: libraryGames.filter(g => g.status === 'Dropped').length,
  };

  const chartData = [
    { name: t('Playing'), value: stats.playing, color: '#22d3ee' },
    { name: t('Completed'), value: stats.completed, color: '#34d399' },
    { name: t('Backlog'), value: stats.backlog, color: '#e879f9' },
    { name: t('Dropped'), value: stats.dropped, color: '#f87171' },
  ...
  const sensors = useSensors(
    useSensor(PointerSensor, {
      activationConstraint: { distance: 5 }, // 5px tolerans
    })
  );
  ...
  const handleDragEnd = (event) => {
    ...
    const newStatus = over.id; // Sütunun id'si status ile aynı
  ...
  const KANBAN_COLUMNS = [
    ...
    { id: 'Playing', title: t('Playing') },
    { id: 'Completed', title: t('Completed') },
    { id: 'Dropped', title: t('Dropped') }
  ];
    ...
    <div className="py-4 space-y-8 overflow-hidden">
      ...
      <section className="bg-black/5 dark:bg-white/5 backdrop-blur-2xl p-6 rounded-3xl border border-black/5 dark:border-white/10 shadow-2xl relative overflow-hidden group">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
          <motion.div initial={{ scale: 0.5, rotate: 0 }} animate={{ scale: 1, rotate: 45 }} transition={{ type: "spring", stiffness: 260, damping: 20 }} className="relative shrink-0">
          ...
          <div className="flex-1 w-full space-y-4">
            <div className="flex justify-between items-end">
              ...
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={pickRandomGame} className="group relative bg-black dark:bg-white/10 hover:bg-yellow-500 p-3 rounded-2xl transition-all shadow-lg border border-white/5" title={t('WhatToPlay')}>
            ...
            <div className="h-3 w-full bg-black/20 dark:bg-white/10 rounded-full overflow-hidden p-0.5">
              <motion.div initial={{ width: 0 }} animate={{ width: `${xpPercentage}%` }} transition={{ duration: 1.5, ease: "easeOut" }} className="h-full bg-gradient-to-r from-yellow-600 to-yellow-400 rounded-full shadow-[0_0_15px_rgba(234,179,8,0.5)] transition-all duration-1000 ease-out"></motion.div>
            </div>
          </div>
        </div>
      </section>
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <input type="text" placeholder={t('SearchVault')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-black/5 dark:bg-white/5 backdrop-blur-md text-gray-900 dark:text-gray-100 px-5 py-3 rounded-xl border border-black/5 dark:border-white/10 focus:ring-2 focus:ring-yellow-500 outline-none" />
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-[#f0f0f5] dark:bg-[#0a0a0a] text-gray-900 dark:text-white px-4 py-3 rounded-xl border border-black/5 dark:border-white/10 outline-none cursor-pointer shadow-inner">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-3">
            {allAchievements.map((ach, index) => (
              <motion.div key={ach.id} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: index * 0.03 }} whileHover={{ scale: 1.15, zIndex: 50 }} className="relative group cursor-help">
                <div className={`aspect-square rounded-2xl border transition-all duration-500 flex items-center justify-center text-2xl ${ach.isUnlocked ? 'bg-yellow-500/10 border-yellow-500/40 shadow-[0_0_15px_rgba(234,179,8,0.1)]' : 'bg-black/5 dark:bg-white/5 border-transparent grayscale opacity-20'}`}>
                  {ach.icon}
                </div>
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-40 p-3 bg-gray-900/95 backdrop-blur-2xl text-white rounded-2xl opacity-0 group-hover:opacity-100 transition-all pointer-events-none z-[100] shadow-2xl border border-yellow-500/30">
                  <div className="text-center">
                    <p className="text-[10px] font-black text-yellow-500 uppercase tracking-widest mb-1 leading-none">{t(`ach_${ach.id}_title`)}</p>
                    <p className="text-[9px] text-gray-300 leading-tight">{ach.isUnlocked ? t(`ach_${ach.id}_desc`) : "???"}</p>
                  </div>
                  <div className="absolute top-full left-1/2 -translate-x-1/2 border-8 border-transparent border-t-yellow-500/30"></div>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="lg:col-span-4 bg-white/50 dark:bg-black/40 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-black/5 dark:border-white/10">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-3"><span className="w-2 h-6 bg-yellow-500 rounded-full"></span>{t('VaultStats')}</h3>
          <div className="w-full flex items-center justify-center min-h-[250px]">
            {showChart && (
             <ResponsiveContainer width="100%" height={250}>
              ...
              <Tooltip contentStyle={{ backgroundColor: 'rgba(10, 10, 10, 0.8)', backdropFilter: 'blur(10px)', borderRadius: '12px', border: 'none', color: '#fff' }} />
      ...
      {libraryGames.length === 0 ? (
        <div className="py-20 text-center text-gray-500 border-2 border-dashed border-gray-400 rounded-3xl bg-white/5">
          <p className="text-xl font-bold text-gray-900 dark:text-white">{t('VaultEmpty')}</p>
          <button onClick={() => navigate('/')} className="mt-4 text-yellow-500 font-bold hover:underline">{t('HeadDiscover')}</button>
        </div>
      ) : (
          ...
          <div className="flex overflow-x-auto pb-8 pt-4 gap-6 snap-x hide-scrollbar">
            {KANBAN_COLUMNS.map(col => (
              <KanbanColumn 
                ...
                id={col.id}
                title={col.title}
                games={sortedAndFilteredGames.filter(g => g.status === col.id)}
