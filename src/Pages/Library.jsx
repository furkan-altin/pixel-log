...
const Library = () => {
  ...
  return (
    <div className="py-4 space-y-8 overflow-hidden">
      ...
      {/* LEVEL HEADER */}
      <section className="bg-black/5 dark:bg-white/5 backdrop-blur-2xl p-6 rounded-3xl border border-black/5 dark:border-white/10 shadow-2xl relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
          <motion.div initial={{ scale: 0.5 }} animate={{ scale: 1 }} className="relative shrink-0">
          ...
          <div className="flex-1 w-full space-y-4">
            <div className="flex justify-between items-end">
              ...
              <motion.button whileHover={{ scale: 1.05 }} onClick={pickRandomGame} className="bg-black dark:bg-white/10 p-3 rounded-2xl border border-white/5">
                <span className={isRolling ? "animate-spin block" : ""}>🎲</span>
              </motion.button>
            </div>
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <input type="text" placeholder={t('SearchVault')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-black/5 dark:bg-white/5 px-5 py-3 rounded-xl border border-black/5 dark:border-white/10 outline-none" />
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-[#f0f0f5] dark:bg-[#0a0a0a] text-gray-900 dark:text-white px-4 py-3 rounded-xl outline-none cursor-pointer">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 bg-white/50 dark:bg-black/40 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-black/5 dark:border-white/10">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-3"><span className="w-2 h-6 bg-yellow-500 rounded-full"></span>{t('Achievements')}</h3>
          <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
            {allAchievements.map((ach) => (
              <div key={ach.id} className={`aspect-square rounded-2xl border flex items-center justify-center text-2xl ${ach.isUnlocked ? 'bg-yellow-500/10 border-yellow-500/40' : 'bg-black/5 dark:bg-white/5 grayscale opacity-20'}`}>
                {ach.icon}
              </div>
            ))}
          </div>
        ...
        <div className="flex overflow-x-auto pb-8 pt-4 gap-6 snap-x hide-scrollbar">
          {KANBAN_COLUMNS.map(col => (
            <KanbanColumn 
              ...
              onStatusChange={updateGameStatus} // FONKSİYONU BURADA GEÇİYORUZ
              t={t}
            />
          ))}
        </div>
      ...
      <AnimatePresence>
        {randomGame && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[150] bg-black/95 backdrop-blur-2xl flex items-center justify-center p-4">
            <div className="bg-white/5 border border-yellow-500/50 p-8 rounded-[40px] max-w-md w-full text-center">
              <h3 className="text-2xl font-black text-white mb-8">{randomGame.title}</h3>
              <button onClick={() => { updateGameStatus(randomGame.id, 'Playing'); setRandomGame(null); toast.success(t('FateAccepted')); }} className="w-full bg-yellow-500 text-black font-black py-4 rounded-2xl mb-2">{t('AcceptFate')}</button>
              <button onClick={() => setRandomGame(null)} className="w-full bg-white/10 text-white font-bold py-3 rounded-2xl">{t('Close')}</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
